function [indmin mdall notenough]=multimodelnlm_global_novalid(Age_gender,Brainsz,data_r,ylabelname,Plotornot,betarealinitial)
notenough=[];
...
if(Plotornot==1)
    plot4fit(Age,Data_rremain,fitresult1,name,ylabelname,rpall(1,1),rpall(2,1))
end
...
if(Plotornot==1)
plot4fit(Age,Data_rremain,fitresult2,name,ylabelname,rpall(1,2),rpall(2,2))
end
...
[mdl3 fitresulte3 gofite rpall(:,3)]=Curve_fitting_nlm(mdl2,modelnlmreal,modelfitreal,Lowerboundary,Upperboundary,ylabelname,name,Plotornot,betarealinitial);
seor=1;
